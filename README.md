...
- HISTORICAL CONTROL: deterministic seeded sampler implemented; the current local French 2018–2021 sample contains 1,000 records / 227,358 words; party metadata is unknown for 775 records, and no other language control sample exists yet ([method and QA](docs/historical_controls.md))
...
- GERMANY: official XML pagination and parser, one 2026-09-23 sample with 190 turns / 43,805 words. The full acquisition stopped when the Bundestag redirected scripted listing/download requests to an Enodia verification challenge (HTTP 400); partial raw files are retained locally. No complete German corpus exists ([sample QA](docs/germany_sample_qa.md)).
...
- COST CONTROL: per-item rounded estimator implemented; local length-eligible estimates are **$42,397.05** for France, **$26,924.80** for Italy, **$26,680.00** for the unreconciled Dutch corpus, and **$14,813.10** for the Spanish corpus with two unresolved journals. None is an account quote or a validated six-country total.
...
The API key is **not** the only blocker: German, Spanish, and Polish acquisition or gap resolution, Dutch meeting reconciliation, full six-country coverage validation, licensing/third-party processing review, and random manual boundary audits remain unfinished. A guard prevents paid submission when any country/year is missing or recorded Spanish journal gaps remain; it cannot substitute for source QA. Do not cite mock outputs as research findings. Detailed earlier French counts and anomalies are in [docs/corpus_qa.md](docs/corpus_qa.md).
