...
|---|---|---|---|
| Germany — Bundestag | Official Bundestag Open Data XML protocol lists for terms 19–21; DIP REST is an alternative.[4][5] The verified fallback baseline is CPP-BT 2026-09-19, a CC0 research compilation derived from official XML/DIP records.[24] | CPP-BT speech archive: 64,796 non-empty records / 32,514,063 words, latest sitting 2026-09-11; full-file integrity QA passed. [Build QA](germany_corpus_qa.md); [sample QA](germany_sample_qa.md). Earlier XML requests met Enodia verification, but ordinary GETs for 23 and 24 September returned XML on 25 September; the 24 September index entry says *partial protocol*. | Incorporate and validate final official protocols after the archive cutoff, reconcile session counts and perform human boundary review. The paid guard still blocks the recorded gap. |
