...
| Italy — Camera dei deputati | Official open-data portal provides RDF data.[12] For stenographic records, the Camera publishes HTML at `camera.it/leg{term}/410?idSeduta={id}&tipo=stenografico`; indexed pages also expose PDFs.[18] | A 2019 official PDF and a live 2026 sitting HTML page were retrieved. The explicit-sitting HTML adapter parsed 47 interventions / 1,808 words from 2026-09-18; details are in `docs/italy_sample_qa.md`. | Adapter handles a specified sitting ID, but complete session-index discovery and the 2018–2026 archive run remain outstanding. Speaker/party enrichment, random source-boundary validation, and reuse/third-party processing terms must be reviewed; do not use OCR without validation. |
