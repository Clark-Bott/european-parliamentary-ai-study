...
|---|---|---|---|
| Germany — Bundestag | Official Bundestag Open Data XML protocol lists for terms 19–21 (no DIP key required for the sample); DIP REST is an alternative.[4][5] | The 2026-09-23 XML sample contains 190 speaker turns / 43,805 words. [Sample QA](germany_sample_qa.md). Full acquisition failed after partial downloads: on 2026-09-24 the list request returned HTTP 303 to `/.enodia/challenge` then HTTP 400 (verification page), confirmed with a direct HTTP-header check. | Obtain a permitted archive/API access route; do not treat the cached partial term-19 XML as complete. Then perform random source review and rights review. |
...
| Netherlands — Tweede Kamer | Official VLOS-derived `Verslag` downloadable XML; OData JSON metadata. The [portal disclaimer](https://opendata.tweedekamer.nl/disclaimer) states CC0 1.0 applies to its API data unless otherwise indicated, and notes IP-level bandwidth limits.[9][10][11] | The adapter selects a corrected final report when present and otherwise the latest final publication; it records status on each turn. One corrected 2025-03-19 report produced 308 interventions / 55,561 words.[23] | Full 2018–2026 download, correction-status coverage audit, random source review, and QA remain outstanding. Verify no record-specific exception to CC0. |
