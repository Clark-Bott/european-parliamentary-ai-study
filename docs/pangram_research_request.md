...
The pipeline also supports a conservative, [budgeted country-month sample](budgeted_inference.md) with inverse-inclusion-weighted plots and a separately authorized **one-block synthetic paid API test**. Both are software-tested with fake responses; no live paid API task has been run. The API test can establish basic key/task/caching behavior for about $0.05 at the public default rate without sending parliamentary text. The budgeted parliamentary option **still requires** full source validation and processing approval; small samples produce exploratory estimates, not precise population rates.
