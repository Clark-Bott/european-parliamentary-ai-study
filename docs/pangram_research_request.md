...
This is **not yet** a claim that “the dataset, methodology, API integration, and analysis pipeline are all built.” France is the only chamber with a prior full parsed corpus; all six have source adapters but the other five have only single-sitting checks. Full cross-country acquisition, quality review, licensing/third-party processing review, and random source-boundary validation remain necessary. Pangram API access was intentionally not used; the default dry run uses clearly labeled synthetic fixtures only.
